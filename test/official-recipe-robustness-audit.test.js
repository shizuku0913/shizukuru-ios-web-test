...
function variants(record){
  ...
  for(let i=0;i<record.recipe.length;i++) for(const pct of [-0.20,-0.10,0.10,0.20]){
  ...
  for(const delta of [-0.10,-0.05,0.05,0.10]) out.push({kind:'water',label:`water ${delta>0?'+':''}${delta.toFixed(2)}`,recipe:record.recipe,water:record.waterLevel+delta});
  return out;
}

test('audit: official recipe robustness under small ingredient and water perturbations',()=>{
  const rows=[];
  ...
  const report={version:'10.8.38-official-recipe-robustness-audit',officialCount:rows.length,totalPerturbations:all,successfulPerturbations:pass,successRate:pass/all,fragileColors:rows.filter(r=>r.rate<1).length,worst:sorted.slice(0,50),rows};
  fs.writeFileSync('OFFICIAL-RECIPE-ROBUSTNESS-AUDIT.json',JSON.stringify(report,null,2));
  const md=['# Shizukuru Official Recipe Robustness Audit — v10.8.38','',`- Official colors: ${rows.length}`,`- Perturbations: ${all}`,`- Intended color retained: ${pass}/${all} (${(100*pass/all).toFixed(2)}%)`,`- Colors with at least one boundary crossing: ${report.fragileColors}`,'','## Lowest robustness', '',...sorted.slice(0,30).map((r,i)=>`${i+1}. ${r.name} — ${(100*r.rate).toFixed(1)}% (${r.pass}/${r.total}); ${r.failures.slice(0,4).map(f=>`${f.label}→${f.actual??'none'}`).join(', ')}`)];
  fs.writeFileSync('OFFICIAL-RECIPE-ROBUSTNESS-AUDIT.md',md.join('\n'));
